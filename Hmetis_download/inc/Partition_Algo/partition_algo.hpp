...
void FM::getEdgePartitionStatus()
{
    for (int i=0; i<graph->getAllEdges().size(); i++)
    {
        ...
        {
            if (inst->getPartitionOf(*itt) == 0)
                pv[0][i]++; //pv[0][i] is the number of nodes of edge i in partition 0
            else
                pv[1][i]++;
        }
    }
...
void FM::updateDataStructure(int n_idx)
{
    ...
    partSize[partTo] += graph->getNodeSizeOf(n_idx);
    partSize[part] -= graph->getNodeSizeOf(n_idx);
...
int FM::selectVictimNode()
{
    ...
    while (noImprove <= k && tmpMaxGain > (-1)*maxGain)
    {
        ...
        for (auto it=gainBucket[bucketIdx].begin(); it!=gainBucket[bucketIdx].end(); it++)
        {
            ...
            if (partSize[partTo]+graph->getNodeSizeOf(*it) > spaceLimit) continue;
...
void biPart(Hypergraph* graph, LayerInfo* inst, int spaceLimit, string& scheme)
{
    if (scheme == "FM")
    {
