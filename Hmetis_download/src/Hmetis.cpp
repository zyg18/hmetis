...
void Hmetis::configureGraph(BOX &box)
{
    ...
    cout<<"node num:"<<box.instLib.size()-1<<endl;
    cout<<"edge num:"<<box.net.size()-1<<endl;
    ...
    {
        tmpNodeSize = 1;
        totalNodeSize += tmpNodeSize;
        graph->setNodeSizeOf(i-1, tmpNodeSize); //设置了每个点的大小为1
    }

    spaceLimit=graph.nodeNum*double(box.Maxpercent)/100.0;
