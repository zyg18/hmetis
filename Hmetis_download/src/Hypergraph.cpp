...
// return node size
double HyperNode::getNodeSize()
{
    return size;
}

// set node size
void HyperNode::setNodeSize(double n)
{
    size = n;
}
...
double Hypergraph::getNodeSizeOf(int n)
{
    return nodes[n]->getNodeSize();
}
...
void Hypergraph::setNodeSizeOf(int idx, double size)
{
    nodes[idx]->setNodeSize(size);
}
...
void Hypergraph::revertGraph(std::vector<std::pair<int, std::vector<int>*>>* coarsenInfo)
{
    ...
    {
        int base = it->first;
        double size0 = getNodeSizeOf(base), size1 = getNodeSizeOf(base);
        ...
        for (auto itt=nds->rbegin(); itt!=nds->rend(); itt++)
        {
            ...
            size0 -= getNodeSizeOf(*itt);
            size1 -= getNodeSizeOf(*itt);
        }

        setNodeSizeOf(base, size0);
        setNodeSizeOf(base, size1);
...
bool LayerInfo::bfsPlacing(Hypergraph* graph, int spaceLimit)
{
    ...
    {   
        ...
        while (p1size > spaceLimit)
        {
            ...
            double size0 = graph->getNodeSizeOf(top), size1 = graph->getNodeSizeOf(top);
            if (!(p0size+size0 > spaceLimit))
            {
